...
# ============================================================================
# CORS - CORRECCIÓN: Agregar tu dominio de Vercel
# ============================================================================

ALLOWED_ORIGINS = [
    "http://localhost:3000",
    "http://localhost:5173",
    "https://productivity-assistant-self.vercel.app",
]

if os.getenv("ALLOW_ORIGINS"):
    ALLOWED_ORIGINS.extend(os.getenv("ALLOW_ORIGINS").split(","))
...
def _save_store(store: dict):
    try:
        ...
        print(f"Error guardando conversations.json: {e}")
        raise
...
def load_conversation(conversation_id: Optional[str]) -> ConversationState:
    ...
    if isinstance(raw, dict):
        ...
        if "pending_calendar_proposal" not in raw:
...
# ============================================================================
# LÓGICA PRINCIPAL: ANÁLISIS CON GEMINI
# ============================================================================
...
"""
    ...
    try:
        ...
        error_msg = f"Error procesando la propuesta con Gemini: {str(e)}"
...
async def chat(request: ChatRequest):
    ...
    3. Llamar a Gemini para analizar
    4. Generar propuesta
    ...
    try:
        ...
        if request.create_calendar_event:
            ...
            if event and request.total_duration_minutes:
                try:
                    ...
                    error_msg = f"Error generando propuesta de calendario: {str(e)}"
                    print(f"[ERROR] {error_msg}")
                    return {
                        "success": False,
                        "error": error_msg,
        ...
        # Llamar a Gemini para generar propuesta
        try:
        ...
        return {
            ...
            "proposed_changes": proposal.model_dump()
        }
    
    except HTTPException:
        raise
    except Exception as e:
        error_msg = f"Error en /api/chat: {str(e)}"
        print(f"[ERROR] {error_msg}")
        raise HTTPException(status_code=500, detail=error_msg)
...
async def adjust_proposal(request: AdjustProposalRequest):
    ...
    try:
        ...
        error_msg = f"Error ajustando propuesta: {str(e)}"
        print(f"[ERROR] {error_msg}")
        raise HTTPException(status_code=500, detail=error_msg)
...
async def confirm_calendar_proposal(conversation_id: str):
    ...
    try:
        ...
        error_msg = f"Error confirmando propuesta: {str(e)}"
        print(f"[ERROR] {error_msg}")
        raise HTTPException(status_code=500, detail=error_msg)
...
if __name__ == "__main__":
    ...
    print("[INFO] Iniciando Productivity Assistant Backend")
    print(f"[INFO] CORS permitidos: {ALLOWED_ORIGINS}")
    print(f"[INFO] GEMINI_API_KEY configurada: {'GEMINI_API_KEY' in os.environ}")
